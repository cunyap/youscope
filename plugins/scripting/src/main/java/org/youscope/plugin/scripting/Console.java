...
class Console extends JPanel implements ScriptMessageListener {
	...
	private void evalString(String script) {
		...
		// Built-in console commands. They are handled before sending to the engine.
		String trimmed = script.trim().toLowerCase();
		if (trimmed.equals("clear") || trimmed.equals("clc") || trimmed.equals("cls")) {
			clearConsole();
			return;
		}
	...
	private class MultipleInputActionListener implements ActionListener {
		@Override
		public void actionPerformed(ActionEvent e) {
			String input = inputAreaArea.getText();
			if (input.length() <= 0)
				return;
			inputAreaField.setText("");
			evalString(input);
