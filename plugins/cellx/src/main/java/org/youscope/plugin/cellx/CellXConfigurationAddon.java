...
class CellXConfigurationAddon extends ComponentAddonUIAdapter<CellXConfiguration> {
	...
	private final JFormattedTextField coresField = new JFormattedTextField(
	...
	static ComponentMetadataAdapter<CellXConfiguration> getMetadata() {
		return new ComponentMetadataAdapter<CellXConfiguration>(CellXConfiguration.TYPE_IDENTIFIER,
				...
				"Uses the CellX algorithm to segment cells, and returns the segmentation image and the cells' positions and properties.",
	...
	protected Component createUI(CellXConfiguration configuration)
		...
		selectConfigButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				JFileChooser fileChooser = new JFileChooser(
						getClient().getPropertyProvider().getProperty(PROPERTY_LAST_CELLX_CONFIGFILE, ""));
				...
				while (true) {
					...
					if (!file.exists()) {
						JOptionPane.showMessageDialog(null, "File " + file.toString() + " does not exist.",
								"File does not exist", JOptionPane.INFORMATION_MESSAGE);
					} else
						break;
				}
		...
		coresField.setColumns(5);
		coresField.setValue(1); // default 1 core
		coresPanel.add(coresField, BorderLayout.CENTER);
		JLabel coresLabel = new JLabel("Number of cores to use for segmentation:");
		centralPanel.add(coresLabel);
		centralPanel.add(coresPanel);
		...
		coresField.setValue(configuration.getNumCores());
	...
	private void configureCellX() {
		...
		if (!cellxDirectory.exists() || !cellxDirectory.isDirectory()) {
			sendErrorMessage("Directory " + cellxDirectory.getAbsolutePath()
					+ " does not exist. Check the CellX addon installation for consistency.", null);
			return;
		}

		File cellxFile = new File(cellxDirectory, "CellXGui.jar");
		if (!cellxFile.exists() || !cellxFile.isFile()) {
			sendErrorMessage("CellX Gui " + cellxFile.getAbsolutePath()
					+ " does not exist. Check the CellX addon installation for consistency.", null);
			return;
		}
		try {
	 ...
	 * LabelVisualizerConfiguration visualizationConfiguration = new
	 * LabelVisualizerConfiguration();
	 * 
	 * // Get algorithms for visualization and detection
	 * ServiceLoader<CellVisualizationAddonFactory> visualizationAddonFactories =
	 * ServiceLoader.load(CellVisualizationAddonFactory.class,
	 * CellDetectionMeasurementConstructionAddon.class.getClassLoader());
	 * CellDetectionAddon detectionAlgorithm = new CellXAddon(new
	 * PositionInformation(), detectionConfiguration);
	 ...
	 * {
	 * if(addonFactory.supportsCellVisualizationAddonID(LabelVisualizerConfiguration
	 * .CONFIGURATION_ID))
	 * {
	 * visualizationAlgorithm =
	 * addonFactory.createCellVisualizationAddon(LabelVisualizerConfiguration.
	 * CONFIGURATION_ID);
	 * break;
	 * }
	 * }
	 ...
	 * {
	 * throw new SettingException("Cell visualization algorithm with ID "
	 * +LabelVisualizerConfiguration.CONFIGURATION_ID + " could not be found.");
	 * }
	 * 
	 ...
	 * {
	 * throw new SettingException("Could not initialize cell detection algorithm.",
	 * e);
	 * }
	 * try
	 * {
	 ...
	 * {
	 * throw new
	 * SettingException("Could not initialize cell visualization algorithm.", e);
	 * }
	 * 
	 * // Detect cells
	 ...
	 * try
	 * {
	 * visualizationResult = visualizationAlgorithm.visualizeCells(orgImage,
	 * detectionResult);
	 * }
	 * catch(CellVisualizationException e)
	 * {
	...
	protected void commitChanges(CellXConfiguration configuration) {
		...
		Number coresValue = (Number) coresField.getValue();
		if (coresValue != null) {
			configuration.setNumCores(Math.max(1, coresValue.intValue()));
		} else {
			configuration.setNumCores(1);
		}
	}
