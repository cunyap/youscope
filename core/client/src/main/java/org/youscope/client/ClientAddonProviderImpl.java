 ...
 *     Andreas P. Cuny - update API supporting post-processor exec without a user interface
 ******************************************************************************/
...
class ClientAddonProviderImpl implements ClientAddonProvider {
	...
	private final List<PostProcessorAddonFactory> postProcessorAddonFactories = new ArrayList<PostProcessorAddonFactory>(
			30);
	...
	public static synchronized ClientAddonProviderImpl getProvider() {
		...
		// post processors
		Iterable<PostProcessorAddonFactory> postProcessorAddonFactories = ServiceLoader.load(
				PostProcessorAddonFactory.class,
				ClientAddonProviderImpl.class.getClassLoader());
	...
	private ComponentAddonFactory getComponentAddonFactory(String typeIdentifier)
		...
		throw new AddonException(
				"No factory to create component addons for type identifer " + typeIdentifier + " exists.");
	}
	...
	public <T extends Configuration> ComponentAddonUI<? extends T> createComponentUI(String typeIdentifier,
		...
		if (factory != null) {
			...
			if (metadata == null) {
				throw new AddonException(
						"Metadata for configuration addon with configuration type " + typeIdentifier + " is null.");
			}
			Class<? extends Configuration> addonClass = metadata.getConfigurationClass();
			if (!configurationClass.isAssignableFrom(addonClass))
				throw new AddonException("Configuration addon with type identifier " + typeIdentifier
						+ " creates configurations of class " + addonClass.getName() + " which are not subclasses of "
						+ configurationClass.getName() + ".");
			@SuppressWarnings("unchecked")
			ComponentAddonUI<? extends T> result = (ComponentAddonUI<? extends T>) factory.createComponentUI(
					typeIdentifier, new YouScopeClientConnectionImpl(), YouScopeClientImpl.getServer());
			return result;
		}
		...
		if (measurementFactory != null) {
			...
			if (!configurationClass.isAssignableFrom(addonClass))
				throw new AddonException("Configuration addon with type identifier " + typeIdentifier
						...
						+ configurationClass.getName() + ".");
			@SuppressWarnings("unchecked")
			ComponentAddonUI<? extends T> result = (ComponentAddonUI<? extends T>) measurementFactory
					.createMeasurementUI(typeIdentifier, new YouScopeClientConnectionImpl(),
	...
	public ComponentMetadata<?> getComponentMetadata(String typeIdentifier) throws AddonException {
		...
		throw new AddonException(
				"Component type identifier " + typeIdentifier + " no supported by any component addon factory.");
	}
	...
	public <T extends Configuration> ComponentMetadata<? extends T> getComponentMetadata(String typeIdentifier,
		...
		if (!configurationClass.isAssignableFrom(metadataRaw.getConfigurationClass()))
			throw new AddonException("Configuration with type identifier " + typeIdentifier + " is of class "
					+ metadataRaw.getConfigurationClass().getName() + ", which is not a subclass of "
					+ configurationClass.getName() + ".");
		@SuppressWarnings("unchecked")
	...
	@Override
	public AddonUI<? extends AddonMetadata> createPostProcessorUI(String typeIdentifier,
		...
		for (PostProcessorAddonFactory addonFactory : postProcessorAddonFactories) {
			if (addonFactory.isSupportingTypeIdentifier(typeIdentifier))
				return addonFactory.createPostProcessorUI(typeIdentifier, new YouScopeClientConnectionImpl(),
						YouScopeClientImpl.getServer(), measurementFileLocations);
		}
		throw new AddonException("Microplate type with type identifier " + typeIdentifier + " is unknown.");
	}

	@Override
	public <T extends AddonMetadata> AddonUI<T> createPostProcessorUI(T metadata,
			MeasurementFileLocations measurementFileLocations) throws AddonException {
	...
	/**
	 * Runs a post-processor without a user interface, e.g. automatically after a
	 * measurement finished.
	 * <p>
	 * Deliberately not part of {@code ClientAddonProvider}: automatic execution is
	 * a client internal mechanism, and adding it to the public interface would
	 * force every
	 * implementation to support it.
	 * </p>
	 * 
	 * @param typeIdentifier           Type identifier of the post-processor.
	 * @param measurementFileLocations Files of the finished measurement.
	 * @throws AddonException Thrown if no installed factory supports the
	 *                        identifier, if the
	 *                        post-processor cannot run unattended, or if it failed
	 *                        to start.
	 */
	public void runAutomaticPostProcessor(String typeIdentifier, MeasurementFileLocations measurementFileLocations)
			throws AddonException {
		for (PostProcessorAddonFactory addonFactory : postProcessorAddonFactories) {
			if (!addonFactory.isSupportingTypeIdentifier(typeIdentifier))
				continue;
			if (!(addonFactory instanceof AutomaticPostProcessorFactory)) {
				throw new AddonException("Post processor with type identifier " + typeIdentifier
						+ " requires a user interface and cannot be run automatically.");
			}
			AutomaticPostProcessorFactory automaticFactory = (AutomaticPostProcessorFactory) addonFactory;
			if (!automaticFactory.isSupportingAutomaticExecution(typeIdentifier)) {
				throw new AddonException("Post processor with type identifier " + typeIdentifier
						+ " does not support automatic execution.");
			}
			automaticFactory.runPostProcessor(typeIdentifier, new YouScopeClientConnectionImpl(),
					YouScopeClientImpl.getServer(), measurementFileLocations);
			return;
		}
		...
		for (PostProcessorAddonFactory addonFactory : postProcessorAddonFactories) {
			for (String algorithmID : addonFactory.getSupportedTypeIdentifiers()) {
				try {
					...
					ClientSystem.err.println("Metadata for post processor with type identifier " + algorithmID
							+ " cannot be constructed. Skipping this post processor.", e);
				}
			}
		}
		return returnVal;
		...
		for (SkinFactory addonFactory : skinFactories) {
			for (String algorithmID : addonFactory.getSupportedTypeIdentifiers()) {
				try {
					...
					ClientSystem.err.println("Metadata for look and feel with type identifier " + algorithmID
							+ " cannot be constructed. Skipping this post processor.", e);
				}
			}
		}
	...
	public ToolAddonUI createToolUI(String typeIdentifier) throws AddonException {
		for (ToolAddonFactory addonFactory : toolAddonFactories) {
			if (addonFactory.isSupportingTypeIdentifier(typeIdentifier))
				return addonFactory.createToolUI(typeIdentifier, new YouScopeClientConnectionImpl(),
						YouScopeClientImpl.getServer());
		}
		...
		for (ToolAddonFactory addonFactory : toolAddonFactories) {
			for (String typeIdentifier : addonFactory.getSupportedTypeIdentifiers()) {
				try {
					...
					ClientSystem.err.println("Metadata for tool with type identifier " + typeIdentifier
							+ " cannot be constructed. Skipping this tool.", e);
				}
			}
		}
		return returnVal;
