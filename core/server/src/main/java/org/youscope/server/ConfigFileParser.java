...
class ConfigFileParser extends ConfigFileManipulator {
    ...
    public ConfigFileParser(MicroscopeInternal microscope, ChannelManagerImpl channelManager,
    ...
    /**
     * Parses a micro manager config file (read in by the reader) and saves its
     * results in the given configuration. If no configuration is supplied,
     * a new one is constructed.
     * 
     * @param reader   A reader reading in the config file.
     * @param accessID Access ID to lock and manipulate the microscope
     *                 configuration.
     * @return The configuration where the information from the config file is
     *         stored in.
     * @throws MicroscopeConfigurationException
     * @throws MicroscopeLockedException
     */
    public ConfigFileParseResult parseConfigFile(Reader reader, int accessID)
            throws MicroscopeConfigurationException, MicroscopeLockedException {
            ...
            try {
                for (;; lineNumber++) {
                    ...
                    if (tokens[0].startsWith("#")) {
                        // Generally, that's a comment line. However, the first comment line might be
                        // useful to identify the generating program (i.e. MicroManager).
                    ...
                    // Image buffer size
                        ...
                        if (!parseImageBufferSize(tokens, accessID)) {
                            parseResult.addWarning("Setting the image buffer size is not supported by this microscope.",
                                    line);
                        }
                    }
                    ...
                    // System startup & shutdown settings
                    else if (isCommand(tokens, COMMAND_CORE_AUTOSHUTTER)
                            || isCommand(tokens, COMMAND_SYSTEM_CORE_AUTOSHUTTER)) {
                        // Ignore this command. Auto-Shutter on/of is a property of the channel in
                        // YouScope.
                        continue;
                    } else if (isCommand(tokens, COMMAND_SYSTEM_STARTUP_CORE)) {
                        parseResult.addWarning(
                                "The MicroManager core device does not exist in YouScope. Only a few settings of it are translated into the respective YouScope settings.",
                                line);
                        continue;
                        ...
                        // Ignore this command: it does not make sense to change core setting one second
                        // before resetting them again...
                        continue;
                    ...
                    else if (isCommand(tokens, COMMAND_PIXEL_SIZE_CAMERA_PITCH))
                        parsePixelSizeCameraPitch(tokens, accessID);
                    else if (isCommand(tokens, COMMAND_PIXEL_SIZE_OBJECTIVE_MAG))
                        parsePixelSizeObjectiveMag(tokens, accessID);
                    else if (isCommand(tokens, COMMAND_PIXEL_SIZE_ADDITIONAL_MAG))
                        parsePixelSizeAdditionalMag(tokens, accessID);
                    ...
                    // Channel settings
                        ...
                        parseResult.addWarning("Only System.Startup and System.Shutdown interpreted by YouScope.",
                                line);
     ...
     * and for every command unequal null the token equals the corresponding
     * command.
     * 
    ...
    private void parseStageUnits(String[] tokens, int accessID)
            throws NumberFormatException, MicroscopeLockedException, DeviceException {
        microscope.getStageDevice(tokens[1]).setUnitMagnifier(Double.parseDouble(tokens[2]), accessID);
    }

    private void parseChannelAutoShutter(String[] tokens, int accessID)
            throws MicroscopeLockedException, SettingException {
        if (tokens[2].endsWith("_on"))
            tokens[2] = tokens[2].substring(0, tokens[2].length() - 3);

        // We are only interested in the inactivation of the auto-shutter. Each channel
        // is anyway
        // configured to have the default shutter as autoshutter, thus only activating
        // the autoshutter without defining which one
    ...
    private void parseChannelAutoShutterDevice(String[] tokens, int accessID)
            throws MicroscopeLockedException, SettingException {
        if (tokens[2].endsWith("_on"))
            tokens[2] = tokens[2].substring(0, tokens[2].length() - 3);
    ...
    private void parseDelay(String[] tokens, int accessID)
            throws NumberFormatException, DeviceException, MicroscopeException, MicroscopeLockedException {
        microscope.getDevice(tokens[1]).setExplicitDelay(Double.parseDouble(tokens[2]), accessID);
    }

    private void parseLabel(String[] tokens, int accessID)
            throws NumberFormatException, MicroscopeException, MicroscopeLockedException, DeviceException {
        microscope.getStateDevice(tokens[1]).setStateLabel(Integer.parseInt(tokens[2]), tokens[3], accessID);
    }

    private boolean parseImageBufferSize(String[] tokens, int accessID)
            throws NumberFormatException, SettingException, MicroscopeLockedException {
        try {
    ...
    private void parseCommunicationTimeout(String[] tokens, int accessID)
            throws NumberFormatException, SettingException, MicroscopeLockedException {
        microscope.getMicroscopeConfiguration().setCommunicationTimeout(Integer.parseInt(tokens[5]), accessID);
    }

    private void parseProperty(String[] tokens, int accessID)
            throws DeviceException, MicroscopeException, MicroscopeLockedException, InterruptedException {
        if (isInitialized) {
    ...
    private void parseUninitialize(int accessID)
            throws MicroscopeException, MicroscopeLockedException, InterruptedException {
        microscope.uninitializeMicroscope(accessID);
        isInitialized = false;
    }

    private void parseInitialize(int accessID)
            throws MicroscopeLockedException, MicroscopeException, InterruptedException {
    ...
    private void parseSystemStartup(String[] tokens, int accessID) throws SettingException, MicroscopeLockedException {
        microscope.getMicroscopeConfiguration()
                .addSystemStartupSetting(new DeviceSetting(tokens[3], tokens[4], tokens[5]), accessID);
    }

    private void parseSystemShutdown(String[] tokens, int accessID) throws SettingException, MicroscopeLockedException {
        microscope.getMicroscopeConfiguration()
                .addSystemShutdownSetting(new DeviceSetting(tokens[3], tokens[4], tokens[5]), accessID);
    }
    ...
    private void parseChannel(String[] tokens, int accessID) throws MicroscopeLockedException, SettingException {
        ...
        // YouScope therefore saves channels for which on and off settings exists as two
        // channels,
    ...
    private void parseChannelDelay(String[] tokens, int accessID)
            throws NumberFormatException, MicroscopeLockedException {
        if (tokens[2].endsWith("_on"))
            tokens[2] = tokens[2].substring(0, tokens[2].length() - 3);

        channelManager.addChannel(tokens[1], tokens[2], accessID).setChannelTimeout(Integer.parseInt(tokens[3]),
                accessID);
    }
    ...
    private void parseNewDevice(String[] tokens) throws SettingException {
        if (isInitialized)
            throw new SettingException(
                    "All devices must be defined after the command \"Property,Core,Initialize,0\" and before the command \"Property,Core,Initialize,1\"");
    ...
    private void parseStandardAutoFocus(String[] tokens, int accessID)
            throws DeviceException, MicroscopeLockedException {
    ...
    private void parseConfigPixelSize(String[] tokens, int accessID)
            throws MicroscopeLockedException, SettingException {
        pixelSizeManager.addPixelSize(tokens[1], accessID)
                .addPixelSizeSetting(new DeviceSetting(tokens[2], tokens[3], tokens[4]), accessID);
    }
    ...
    private void parsePixelSizeCameraPitch(String[] tokens, int accessID)
            throws NumberFormatException, MicroscopeLockedException {
        // Order matters: setting the components triggers recomputation, but
        // PixelSize_um is written
        // before the components in the cfg, so the directly-set value is overwritten by
        // the derived one
        // once all three components have been read. Both agree by construction, so this
        // is harmless.
        pixelSizeManager.addPixelSize(tokens[1], accessID)
                .setCameraPixelPitchMicrons(Double.valueOf(Double.parseDouble(tokens[2])), accessID);
    }

    private void parsePixelSizeObjectiveMag(String[] tokens, int accessID)
            throws NumberFormatException, MicroscopeLockedException {
        pixelSizeManager.addPixelSize(tokens[1], accessID)
                .setObjectiveMagnification(Double.valueOf(Double.parseDouble(tokens[2])), accessID);
    }

    private void parsePixelSizeAdditionalMag(String[] tokens, int accessID)
            throws NumberFormatException, MicroscopeLockedException {
        pixelSizeManager.addPixelSize(tokens[1], accessID)
                .setAdditionalMagnification(Double.valueOf(Double.parseDouble(tokens[2])), accessID);
    }

    private void parseAxisConfiguration(String[] tokens, int accessID)
            throws DeviceException, MicroscopeLockedException {
